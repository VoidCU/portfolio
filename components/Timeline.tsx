...
export default function Timeline() {
  return (
    <section id="experience" className="bg-[var(--c-surface)] border-t border-[var(--c-b1)]">
              ...
              <div className="py-7 grid grid-cols-1 md:grid-cols-[auto_1fr_auto] gap-x-8 gap-y-3 items-start">
                ...
                <div>
                  <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                    <h3 className="font-heading font-bold text-[var(--c-text)] text-xl group-hover:text-[var(--c-accent)] transition-colors">
                      {role}
                    </h3>
                  ...
                  <ul className="mt-4 space-y-2">
                    {items.map(item => (
                      <li key={item} className="flex items-start gap-3 text-[var(--c-muted)] text-sm">
                        <span className="mt-1.5 w-1 h-1 bg-[rgba(74,222,128,0.2)] flex-shrink-0" />
                        <span className="group-hover:text-[var(--c-dim)] transition-colors leading-relaxed">
                          {item}
                        </span>
                      </li>
                    ))}
