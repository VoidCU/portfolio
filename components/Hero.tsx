...
export default function Hero() {
  return (
    <section
      id="home"
      className="relative min-h-screen bg-[#080d08] flex flex-col justify-between pt-14 overflow-hidden"
    >
      {/* Top bar */}
      <div className="w-full border-b border-[rgba(74,222,128,0.06)]" />
      ...
      <div className="flex-1 flex flex-col justify-center max-w-7xl mx-auto w-full px-6 py-16">
        ...
        <div className="animate-fade-up">
          <h1
            className="font-heading font-black text-[#e8fdf0] leading-none tracking-tight"
            style={{ fontSize: 'clamp(3.5rem, 12vw, 11rem)' }}
          >
            SAROJ
          </h1>
          <h1
            className="font-heading font-black text-[#4ade80] leading-none tracking-tight"
            style={{ fontSize: 'clamp(3.5rem, 12vw, 11rem)' }}
          >
            PRASAD
          </h1>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-fade-up delay-300">
          <div className="text-[#86efac] font-mono text-sm tracking-wide flex items-center gap-3">
            <span className="text-[#4d7c5a]">./</span>
            <Typewriter
              options={{
                strings: [
                  'Full-Stack Engineer',
                ...
                delay: 60,
                deleteSpeed: 35,
                wrapperClassName: 'text-[#e8fdf0] font-semibold',
                cursorClassName: 'text-[#4ade80]',
              }}
            />
          </div>
          <div className="label">
      ...
      {/* CTA + stats bar */}
      <div className="border-t border-[rgba(74,222,128,0.08)] animate-fade-up delay-400">
          ...
          <div className="flex items-center gap-8">
            {profile.stats.map(({ value, label }) => (
              <div key={label} className="text-center">
                <p className="font-heading font-black text-[#4ade80] text-lg leading-none">{value}</p>
      ...
      {/* Bottom ticker */}
      <div className="border-t border-[rgba(74,222,128,0.06)] overflow-hidden py-2.5 bg-[#0d140d]">
        <div className="animate-ticker whitespace-nowrap w-max flex items-center gap-0">
          {[...techStack, ...techStack].map((t, i) => (
            <span key={i} className="inline-flex items-center gap-8 px-8">
              <span className="label text-[#1f3a28]">{t}</span>
              <span className="w-1 h-1 bg-[#1f3a28]" />
            </span>
          ))}
        </div>
      </div>
